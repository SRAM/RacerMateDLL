...
TCHAR *CRegExp::reg(int paren, int *flagp)  {
	...
	*flagp = HASWIDTH;	// Tentatively. 
	...
	if (paren) 	{
		...
		if (regnpar >= NSUBEXP)  {
				...
				sprintf(dbgstr, "Too many (). NSUBEXP is set to %d\n", NSUBEXP );
	...
	*flagp |= flags&SPSTART;
	...
	{
		OutputDebugString("unterminated ()\n");
		return NULL;
	} 
	else if (!paren && *regparse != _T('\0')) 
	{
		if (*regparse == _T(')')) 
		{
			OutputDebugString("unmatched ()\n");
			return NULL;
		} 
		else
		{
			OutputDebugString("internal error: junk on end\n");
			return NULL;
		}
		// NOTREACHED 
	}
...
TCHAR *CRegExp::regpiece(int *flagp)  {
	...
	{
		OutputDebugString("*+ operand could be empty\n");
		return NULL;
	}
...
TCHAR *CRegExp::regatom(int *flagp)  {
	...
	*flagp = WORST;		// Tentatively. 
	...
	case _T('['): {
		...
		while ((c = *regparse++) != _T('\0') && c != _T(']')) {
			...
			else 
			{
				...
				if (range > rangeend)
				{
					OutputDebugString("invalid [] range\n");
					return NULL;
				}
	...
	case _T(')'):
		...
		OutputDebugString("internal error: \\0|) unexpected\n");
		return NULL;
		break;
	case _T('?'):
	case _T('+'):
	case _T('*'):
		OutputDebugString("?+* follows nothing\n");
		return NULL;
		break;
	case _T('\\'):
		if (*regparse == _T('\0'))
		{
			OutputDebugString("trailing \\\n");
			return NULL;
		}
		ret = regnode(EXACTLY);
		regc(*regparse++);
	...
	default: {
		...
		if (len == 0)
		{
			OutputDebugString("internal error: strcspn 0\n");
			return NULL;
		}
		ender = *(regparse+len);
...
int CRegExp::RegFind(const TCHAR *str)  {
	...
	if(string == NULL) 
	{
		OutputDebugString("NULL argument to regexec\n");
		return(-1);
	}
	...
	if (!bCompiled) 
	{
		OutputDebugString("No regular expression provided yet.\n");
		return(-1);
	}
...
int CRegExp::regmatch(TCHAR *prog)  {
	...
	for (scan = prog; scan != NULL; scan = next) {
		...
		case END:
			...
			break;
		default:
			OutputDebugString("regexp corruption\n");
			return(0);
			break;
		}
	}
	...
	OutputDebugString("corrupted pointers\n");
	return(0);
}
...
size_t CRegExp::regrepeat(TCHAR *node)  {
	...
	default:		// Oh dear.  Called inappropriately. 
		OutputDebugString("internal error: bad call of regrepeat\n");
...
/*
Here's a function that will do global search and replace using regular expressions. Note that the CStringEx 
class described in the earlier section is being used here. The main reason for using CStringEx is that it 
provides the Replace() function which makes our task easier. 
					 ...
					 LPCTSTR sReplaceExp )
{
	...
	while( (nPos = r.RegFind((LPTSTR)str)) != -1 )  {
		...
		string.Replace( offset, r.GetFindLen(), pReplaceStr );
