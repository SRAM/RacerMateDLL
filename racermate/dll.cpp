...
int start_ant(int _debug_level) {
	#ifdef LOGDLL
		logstream = fopen(DLL_LOGFILE, "a+t");
		fprintf(logstream, "start_ant()\n");
		fclose(logstream);
	#endif

	try {
		ant = new RACERMATE::ANT(_debug_level);							// calls libusb_init(), creates and starts worker
	}
	catch(int i) {
		//bp = 2;
	#ifdef LOGDLL
		logstream = fopen(DLL_LOGFILE, "a+t");
		fprintf(logstream, "start_ant caught %d\n", i);
		fclose(logstream);
	#endif
		return i;
	}
	...
	#ifdef LOGDLL
		logstream = fopen(DLL_LOGFILE, "a+t");
		fprintf(logstream, "starting timer\n");
		fclose(logstream);
	#endif

	if(dll_ant_timer_event == 0) {
		dll_tick = 99;										// to force immediate timeout
	#ifdef LOGDLL
		logstream = fopen(DLL_LOGFILE, "a+t");
		fprintf(logstream, "starting timer\n");
		fclose(logstream);
		...
		dll_ant_timer_event = timeSetEvent(1000, 1, ant_timer_proc, (DWORD_PTR)NULL, TIME_PERIODIC);		// generates accurate 1000.0 ms interrupt
	}

	#ifdef LOGDLL
		logstream = fopen(DLL_LOGFILE, "a+t");
		fprintf(logstream, "start_ant x\n");
		fclose(logstream);
	#endif
...
int start_server(int _listen_port,  int _broadcast_port, const char *_myip, int _debug_level)  {
	...
	#ifdef LOGDLL
		logstream = fopen(DLL_LOGFILE, "a+t");
		fprintf(logstream, "start_server(%d, %d, %s, %d)\n", _listen_port, _broadcast_port, _myip, _debug_level);
		fclose(logstream);
	#endif
	...
	#ifdef LOGDLL
		logstream = fopen(DLL_LOGFILE, "a+t");
		fprintf(logstream, "start_server x, nclients = %d, n = %d\n", nclients, n);
		fclose(logstream);
	#endif
...
#ifdef _DEBUG
	printf("dll.cpp: stop_server, deleting server\n");
#endif
...
int racermate_close(void)  {
	...
	timeKillEvent(dll_ant_timer_event);

	if (server) {
		#ifdef _DEBUG
			//printf("dll.cpp / racermate_close, deleting server\n");
		#endif
		#ifdef LOGDLL
			logstream = fopen(DLL_LOGFILE, "a+t");
			fprintf(logstream, "closing server\n");
			FCLOSE(logstream);
		#endif
		DEL(server);
	}
	...
	if(ant) {
		#ifdef _DEBUG
			//printf("dll.cpp / racermate_close: deleting ant\n");
		#endif
		#ifdef LOGDLL
			logstream = fopen(DLL_LOGFILE, "a+t");
			fprintf(logstream, "closing ANT\n");
			FCLOSE(logstream);
		#endif
		DEL(ant);
		bp = 3;
		#ifdef _DEBUG
			//printf("dll.cpp / ant deleted\n");
		#endif
	}
#endif
...
EnumDeviceType GetRacerMateDeviceID(const char *_id)  {
	...
	strncpy(devices[_id].id, _id, sizeof(devices[_id].id) - 1); // devices grows here
	...
	**********************************************************************/
	...
	struct RACERMATE::TrainerData GetTrainerData(const char *_id, int fw) {
		...
		if (what==DEVICE_VELOTRON)  {
			if (fw != 190)  {
			//nca:	throw(fatalError(__FILE__, __LINE__));
			}
		}
	#endif
...
void calc_tp(const char *_ix, int _fw, unsigned long _ms, float _watts) {

	DEVICE *d = &devices[_ix];

	if(d->commtype == TRAINER_IS_CLIENT) {
		if(server) {
			server->calc_tp(d->udpkey, _watts);
		}
	}
	else if(!devices[_ix].ev) {
		devices[_ix].ev->calc_tp_x(_ms, _watts);
	}
...
**********************************************************************/
	...
	#ifdef LOGDLL
		logstream = fopen(DLL_LOGFILE, "a+t");
		fprintf(logstream, "   OK1c, _nfront = %d\n", _nfront);
		fclose(logstream);
	#endif
...
/**********************************************************************
	gets the current udpkey for _appkey from devices[]
	GET IT FROM devices because it is propogated to there from ctsrv in sync()
**********************************************************************/
/*
const char * get_udp_key(const char *_appkey) {

	got
	if(server) {
		return server->get_udpkey_from_devnum(&_appkey[4]);
	}
...
const char * get_rs232_ports(void) {
	char str[32];
	static char comports[48 * 8];										// allocate memory for up to 48 com port names
...
const char * getPortNames(void) {
	...
	ccptr = get_rs232_ports();
	strcpy(portnames, ccptr);
...
const SENSORS *get_ant_sensors(void) {
	if(ant) {
		return ant->get_sensors();
	}
	return NULL;
}								// get_udp_clients()
...
const char *get_ant_sensors_string(void)  {
	if(ant) {
		return ant->get_sensors_string();
	}
	return NULL;
}								// get_udp_clients()

/*********************************************************
	example: associate(38760, "UDP-5555")
*********************************************************/

int associate(const char *_portname, const unsigned short _sn) {
	associations[_portname] = _sn;
	return 0;
}
...
int unassociate(const char *_portname, const unsigned short _sn) {
	int status;

	status = associations.erase(_portname);
	return status;
}
...
unsigned char get_ant_hr(unsigned short _sn) {
	if(ant) {
		return ant->get_hr(_sn);
	}
	return NULL;
}								// get_udp_clients()
...
const RACERMATE::UDPClient::COMSTATS *get_comstats(const char * _id, int _fw)  {
	...
	//if(got == devices.end()) {
		DEVICE *d = &devices[_id];
		if(d->commtype == TRAINER_IS_CLIENT) {
			const char *ccptr = d->udpkey;
			if(server) {
				return server->get_comstats(ccptr);
			}
		}
	//}
