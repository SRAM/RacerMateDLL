...
#ifdef DO_ANT
	//set_use_speed_cadence_sensor(ix, 1);				// now done in start_trainer!!!
	//set_use_heartrate_sensor(ix, 1);					// now done in start_trainer!!!
#endif
...
*/
	...
	// vector::dirs is in glib globals
	...
	tcp_port = ini.getInt("options", "tcp port", 8899, true);
	bp = 2;
   ...
   while( token != NULL )  {
	...
	for(i=0; i<(int)portnames.size(); i++)  {
		...
		/*
		if (!strcmp(portnames[i].c_str(), SERVER_SERIAL_PORT_STR))  {
			port = SERVER_SERIAL_PORT;
			ix = port - 1;
		}

		if (!strcmp(portnames[i].c_str(), CLIENT_SERIAL_PORT_STR))  {
	...
	//tcp_port = 9072;
	//strcpy(url, "miney2.mselectron.net");
...
ix = 34;
	*/
	...
	/*
		...
		RTD::RTD()												// constructor 2
		Computrainer::Computrainer()						// constructor2
		Computrainer::init()

		Decoder::Decoder()
		ComputrainerDecoder::ComputrainerDecoder()
	*/

	if (port==CLIENT_SERIAL_PORT)  {
		status = set_network(ix, url, tcp_port);				// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
		assert(status==ALL_OK);
	}
...
#ifdef DO_ANT
	//set_use_speed_cadence_sensor(ix, 1);
	//set_use_heartrate_sensor(ix, 1);
#endif
...
#endif
			printf("%6.1f  mph=%6.1f  rpm=%6.1f  hr=%6.1f  w=%6.1f  k=%02x  %s\r\n",seconds, mph, rpm, hr, watts, keys&0x000000ff, keystr);
		}
