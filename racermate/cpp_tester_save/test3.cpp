...
//#include <stdio.h>
	...
	/*
	BOOL b;
	int sz = sizeof(BOOL);					// sz = 0x00000004
	b = TRUE;									// b = 0x00000001
	b = FALSE;									// b = 0x00000000
...
void test_drag_factor(void)  {
	...
	if (FAILED(fw))  {
		cptr = get_errstr(fw);
		sprintf(gteststring, "couldn't get firmware: %s", cptr);
		throw(fatalError(__FILE__, __LINE__, gteststring));
	}
	printf("firmware = %d\n", fw);
...
const char *vtkeyname[6] = {
	...
	std::map<int, std::string> ix_to_portname;
		...
		strcpy(port, "/dev/ttyUSB0");				// usb-serial adapter
	...
	if (what == DEVICE_COMPUTRAINER)  {
		...
		sprintf(gteststring, "don't see computrainer on port %d\r\n", ix+1);
		throw(fatalError(__FILE__, __LINE__, gteststring));
	}
	else  {
		sprintf(gteststring, "No Computrainer on port %s", port);
		throw(fatalError(__FILE__, __LINE__, gteststring));
	}
...
void just_run_one_trainer_ergvideo_access(void)  {
	...
	int port;
#endif
	INPUT_TYPE input_type = BAD_INPUT_TYPE;
...
const char *vtkeyname[6] = {
	...
	const char *dll_build_date = get_build_date();
	printf("\r\ndll build date = %s\r\n\r\n", dll_build_date);
	...
	cptr = ini.getString("options", "input", "COM1", true);
...
#ifdef NEWPORT
	//cptr = ini.getString("options", "serial port", "COM1", true);									// "COM35"
	//strncpy(port, cptr, sizeof(port)-1);
	//ix = 0;

	#ifdef WIN32
		int iport, n;
		if ( strstr(cptr, "COM"))  {
			n = sscanf(cptr, "COM%d", &iport);
			assert(n==1);
		...
		for(i=0; i<256; i++)  {
			sprintf(portname, "%d", i+1);
			portname_to_ix[portname] = i;
			ix_to_portname[i] = portname;
		}
		...
		if ( strstr(portname, "COM"))  {
			...
			input_type = UNIX_RS232;
			ix = 0;
		}
		else if ( !strcmp(portname, "tcp"))  {
			input_type = TCP_SERIAL;
			ix = 0;
		}
		else  {
			throw(fatalError(__FILE__, __LINE__));
		...
		//strcpy(portname, "/dev/ttyUSB0");				// usb-serial adapter
	...
	//ix = portname_to_ix[port];
	//ix = portmap[port];
	set_portname(ix, portname);									// only sets the maps, not the device[].portname

	printf("ix = %d, port = %s\r\n", ix, portname);

	if (input_type==TCP_SERIAL)  {
...
#ifdef NEWPORT
	/*
	printf("getting device id, port = %s, ix = %d\r\n", portname, ix);
	int port=-1;
	int n;

	if (strstr(portname, "COM"))  {
		n = sscanf(portname, "COM%d", &port);
		if (n==-1)  {
			bp = 2;
		}
	}
	...
	*/
#else
	printf("getting device id, port = %d, ix = %d\r\n", port, ix);
...
if (!short_circuit)  {
	...
	if (what == DEVICE_COMPUTRAINER)  {
		...
		sprintf(gteststring, "don't see trainer on port %d\r\n", ix+1);
		throw(fatalError(__FILE__, __LINE__, gteststring));
	}
	else  {
#ifdef NEWPORT
...
#else
		sprintf(gteststring, "no trainer on port %d", port);
#endif
		throw(fatalError(__FILE__, __LINE__, gteststring));
	}
