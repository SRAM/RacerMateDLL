...
//void rmp_test1(void);
	...
	void just_run_one_computrainer_ergvideo_access(void);
...
//goto xit;
	...
	#elif LEVEL == DLL_ERGVIDEO_ACCESS
		//rmp_test1();
		just_run_one_computrainer_ergvideo_access();
...
const char *vtkeyname[6] = {
	...
	printf("\r\njust_run_one_computrainer_ergvideo_access(void)\r\n");
   ...
   while( token != NULL )  {
	...
	for(i=0; i<(int)portnames.size(); i++)  {
		printf("%s\n", portnames[i].c_str());
		if (!strcmp(portnames[i].c_str(), "COM253"))  {
#ifndef NEWPORT
			port = 253;
			ix = port - 1;
#endif
	...
	printf("calling GetRacerMateDeviceID\r\n");
...
*/
	...
	return;
}							// just_run_one_computrainer_eegvideo_access()()
