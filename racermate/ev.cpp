...
#ifdef LOGEV
	 sprintf(EVLOGFILE, "%s\\ev%s.log", personal_path, _ix);

	 unlink(EVLOGFILE);
    logstream = fopen(EVLOGFILE, "wt");
    fprintf(logstream, "ev logging 1\n");
    fclose(logstream);
#endif

	 if (what == DEVICE_COMPUTRAINER) {
		 strncpy(comstr, devices[_ix].id, sizeof(comstr) - 1);
		...
		ds = new Computrainer(comstr, rd, course, NULL, logging_type, "", -1);								// comstr is class member, 32 bytes, computrainer constructor 1, rtd constructor 1
       ...
       ds->setShutDownDelay(10);
		...
		#ifdef LOGEV
			logstream = fopen(EVLOGFILE, "a+t");
			fprintf(logstream, "error, what = %d\n", what);
			fclose(logstream);
		#endif
		return;
	}
...
EV::EV(const char *_realportnum, EnumDeviceType _what, RIDER_DATA _rd, Bike::PARAMS *_params, Course *_course, LoggingType _logging_type)  {
	int status;

	if(strstr(_realportnum, "COM") != NULL) {
		realportnum = atoi(&_realportnum[3]);
		if(realportnum == 0) {
...
/*
	 //DEVICE *d = devices.find(id);
	 if(it == devices.end()) {
		 //it = devices.insert(value_type(key, int())).second;
		 bp = 1;
	 }
*/

	 DEVICE *d;
	 d = &devices[id];			// increases devices count! don't do this!!!
	...
	//char *cptr = strstr(devices[id].id, "COM");

	if (strstr(devices[id].id, "COM") != NULL)  {
			bp = 1;
	}
	else if (!strstr(devices[id].id, "/dev/"))  {
		throw(fatalError(__FILE__, __LINE__));
	}
...
#ifdef LOGEV
	 sprintf(EVLOGFILE, "%s\\ev-%s.log", personal_path, _realportnum);

	 unlink(EVLOGFILE);
    logstream = fopen(EVLOGFILE, "wt");
    fprintf(logstream, "ev logging 2\n");
    fclose(logstream);
#endif

    if (what==DEVICE_COMPUTRAINER)  {
        strncpy(comstr, devices[id].id, sizeof(comstr)-1);
        #ifdef LOGEV
				logstream = fopen(EVLOGFILE, "a+t");
	...
	//int i = atoi(id);
	ds->decoder->set_id(d->comport_number);
   ...
   fclose(logstream);
    ...
    FCLOSE(logstream);
	 ...
	 // make a copy of evxx.log
		...
		sprintf(fname,"\\ev_%04d-%02d-%02d_%02d-%02d-%02d.log",x.wYear, x.wMonth, x.wDay, x.wHour, x.wMinute, x.wSecond);
		strcat(dest, fname);
...
RACERMATE::TrainerData * EV::myread(void)  {
	...
	*/
    ...
    if (what==DEVICE_COMPUTRAINER && (ds->decoder->meta.hrflags&0x0c)==0x0c)  {				// if bits 2&3 set, no sensor is installed
#ifdef DO_ANT
        if (asi)  {
            td.HR = hr.get_val();
...
#ifdef LOGEV
logstream = fopen(EVLOGFILE, "a+t");
fprintf(logstream, "accum_hr= %.2f  accum_hr_count= %d  td.HR= %.2f\n", ds->decoder->accum_hr, ds->decoder->accum_hr_count, td.HR);
fclose(logstream);
#endif
*/
    //-------------------------------------
...
#ifdef LOGEV
		logstream = fopen(EVLOGFILE, "a+t");
		fprintf(logstream, "set_paused(%s)\n", _pause?"true":"false");
		fclose(logstream);
#endif
...
#ifdef LOGEV
		logstream = fopen(EVLOGFILE, "a+t");
		fprintf(logstream, "calling ds->pause()\n");
		fclose(logstream);
#endif
        ds->pause();
    }
...
void EV::calc_tp_x(unsigned long _ms, float _watts) {
	if(formula) {
		formula->calc(_watts);
	}
	return;
...
int EV::start_collection_thread(void)  {
    ...
    **********/
    ...
    contin = true;
    if (devices[id].commtype==WIN_RS232)  {
