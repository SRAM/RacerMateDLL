...
RTD::RTD(
	...
	/*
	if (_dataFileName)  {
		strncpy(dataFileName, _dataFileName, sizeof(dataFileName)-1);
	}
	else {
	...
	*/
	...
	//rinf.hr_beep_enabled = (int)rd.hr_beep_enabled;					// bool
	//rinf.draft_enable = (int)rd.draft_enable;						// bool
	...
	for(int i=0;i<CLEN;i++)  workbuf[i] = 0;
			...
			int status = port->Open(_comportstr, &dcs);
		   if(status < 0) {
				port->set_try_usb(true);
		      port->set_portnum(prt);
		
		      status = port->Open(_comportstr, &dcs);
		      if (status < 0)  {
					DEL(port);
		         return;
		      }
			...
			*/
			bp = 1;
			if (log) log->write(10,0,1,"rtd: 3\n");
			try  {
				/*
				if (!strcmp(_comportstr, SERVER_SERIAL_PORT_STR))  {				// "COM201"
					server = new Server();
				}
				*/
				...
				//if (!strcmp(_comportstr, CLIENT_SERIAL_PORT_STR))  {
					client = new Client(url, tcp_port);											// the url and port still need to be set up for this instantiation
				}
				else  {
				*/
				...
				if (log) log->write(10,0,1,"rtd: 3a\n");
...
RTD::RTD(
	...
	server = NULL;
	port = NULL;
	caller_created_port = true;										// constructor 4
