	...
	#include <stdio.h>
	#include <windows.h>						// needed for MAXPATH in tdefs.h
	#include <time.h>
#endif
...
Client::Client(const char *_url, int _port, int _dummy)  {
	int status;
	...
	//int bp = 0;
	calls++;
	if (calls==2)  {						// <<< check_for_windows_client_computrainer()
		bp = 1;
	}
#endif
...
Client::~Client()  {
	...
	if (logstream)  {
		fprintf(logstream, "   destructor(), instances = %d, dummy = %d, id = %d\n", instances, dummy, id);
		FCLOSE(logstream);
...
int Client::init(void)  {
	...
	if (logstream)  {
		...
		fprintf(logstream, "   instances = %d\n", instances);

		if (dummy==1)  {
			fprintf(logstream, "   dummy = %d: created by check_for_windows_client_computrainer()\n", dummy);
		}
		else if (dummy==2)  {
			fprintf(logstream, "   dummy = %d: created by rtd()\n", dummy);
		}
		else  {
			fprintf(logstream, "   dummy = %d: created by ???\n", dummy);
		}


		fprintf(logstream, "   id = %d\n", id);
		fprintf(logstream, "   cwd = %s\n", cwd);
		fprintf(logstream, "   url = %s\n", url);
		fprintf(logstream, "   tcp_port = %d\n", tcp_port);
		fprintf(logstream, "   time = %10ld\n", timeGetTime());
		FCLOSE(logstream);
		logstream = fopen("client.log", "a+t");
	}
...
//	thrd = NULL;
	...
	if (logstream)  {
		fprintf(logstream, "   coin ok (%10ld)\n", timeGetTime());
		fclose(logstream);
		logstream = fopen("client.log", "a+t");
	}
	...
	if ( (status=WSAStartup(0x0202, &wsaData)) != 0 )  {
		if (logstream)  {
			fprintf(logstream, "   error: icc failed (%10ld)\n", timeGetTime());
			fclose(logstream);
			logstream = fopen("client.log", "a+t");
		}
		return 1;
	...
	if (url[0] != 0)  {
		if (logstream)  {
			fprintf(logstream, "   calling gethostbyname() at %10ld\n", timeGetTime());
			fclose(logstream);
			logstream = fopen("client.log", "a+t");
		}
	...
	if (tcp_port != -1)  {
		...
		if (logstream)  {
			fprintf(logstream, "   sending \"Racermate\"\n");
			fclose(logstream);
			logstream = fopen("client.log", "a+t");
		}
		...
		if (logstream)  {
			fprintf(logstream, "   calling expect()\n");
			fclose(logstream);
			logstream = fopen("client.log", "a+t");
		}

		status = expect("LinkUp", 3000L);				// connection refused happens in about 1.5 seconds, so this timeout should be longer than that
...
int Client::close(void) {
	...
	if (logstream)  {
		fprintf(logstream, "   close(): stopping thread\n");
		FCLOSE(logstream);
		logstream = fopen("client.log", "a+t");
	}
...
/*
	if (thrd)  {
		if (logstream)  {
			fprintf(logstream, "   calling join()\n");
			FCLOSE(logstream);
			logstream = fopen("client.log", "a+t");
		}
		thrd->join();
...
*/
	...
	if (logstream)  {
		fprintf(logstream, "   join ok, thrd deleted\n");
		FCLOSE(logstream);
		logstream = fopen("client.log", "a+t");
	}
	...
	if (sock != INVALID_SOCKET)  {
		if (logstream)  {
			fprintf(logstream, "   calling closesocket(%d)\n", sock);
			FCLOSE(logstream);
			logstream = fopen("client.log", "a+t");
		}
	...
	if (dummy==1)  {										// only need to sleep if created by discovery function getRacerMateDeviceId()
		if (logstream)  {
			fprintf(logstream, "   sleeping\n");
			FCLOSE(logstream);
			logstream = fopen("client.log", "a+t");
		}
		Sleep(3000);
...
int Client::expect(const char *str, DWORD timeout)  {
	...
	if (logstream)  {
		fprintf(logstream, "   %10ld: expect 1, str = %s, to = %ld\n", timeGetTime(), str, timeout);
		fclose(logstream);
		logstream = fopen("client.log", "a+t");
	}
	...
	while(1)  {
		if ((timeGetTime() - start) >= timeout)  {
			if (logstream)  {
				fprintf(logstream, "   %10ld: expect() timed out after %ld ms\n", timeGetTime(), timeout);
				fflush(logstream);
				fclose(logstream);
				logstream = fopen("client.log", "a+t");
			}
		...
		if (connect_error != 0)  {
			...
			switch(connect_error)  {						// connect_error is set in thread, return value from connect() calls
		...
		while (rxoutptr != rxinptr)  {
			...
			if (i==EXPECT_BUF_LEN)  {
				if (logstream)  {
					fprintf(logstream, "   i = buflen, calling close() and throwing\n");
					fflush(logstream);
					fclose(logstream);
					logstream = fopen("client.log", "a+t");
				}
		...
		if (!strcmp(str, (const char *)buf))  {
			if (logstream)  {
				fprintf(logstream, "   expect x, received string. GOOD!\n");
				FCLOSE(logstream);
				logstream = fopen("client.log", "a+t");
			}
			return 0;
	...
	if (logstream)  {
		fprintf(logstream, "   expect x, failed to receive string in time. BAD!!!\n");
		fflush(logstream);
		FCLOSE(logstream);
		logstream = fopen("client.log", "a+t");
	}
...
void Client::mythread(void *tmp)  {
	...
	bool flag = false;
	OutputDebugString("   Client thread");
#endif
	...
	while(client->contin)  {
		...
		if (client->sock==INVALID_SOCKET)  {
			...
			//xxx
			//see if I can set connect() timeout
		...
		if (!client->connected_to_server)  {
			...
			if (client->logstream)  {
				fprintf(client->logstream, "      %10ld calling connect()\n", timeGetTime() );
				fclose(client->logstream);
				client->logstream = fopen("client.log", "a+t");
			}
			...
			if (client->logstream)  {
				fprintf(client->logstream, "      %10ld back from connect(), status = %d\n", timeGetTime(), status );
				fclose(client->logstream);
				client->logstream = fopen("client.log", "a+t");
			}
...
finis:
	...
	client->logg(true, "   Client thread x\n");
