...
#ifdef HELO_WRLD
Computrainer::Computrainer(int _ix, RACERMATE::Ctsrv *_server, RIDER_DATA &_rd, Course *_course, Bike *_bike, LoggingType _logging_type ) : RTD(_ix, _server, _rd, _course, false, _bike, _logging_type )   {
#else
Computrainer::Computrainer(int _ix, RIDER_DATA &_rd, Course *_course, Bike *_bike, LoggingType _logging_type) : RTD(_ix, _rd, _course, false, _bike, _logging_type) {
#endif
...
//	}
	...
	if (port)  {
		...
		n = 0;
		//n = server->rx(ix, workbuf, CLEN-2);					// workbuf is 1024 buytes long, CLEN = 1024
	}
#endif
	else if (client)  {														// trainer is a server
...
void Computrainer::pause(void)  {
	...
	decoder->set_paused(true);												// just sets a bool flag in decoder
...
#if 0
	FILE *strm = fopen("xyzzy.txt", "a+t");
	fprintf(strm, "\nComputrainer::pause()\n");

	switch(control_byte)  {
		case HB_WIND_RUN:							// 0x2c
			fprintf(strm, "control_byte is HB_WIND_RUN, setting to HB_WIND_PAUSE\n");
			control_byte = HB_WIND_PAUSE;
			break;

		case HB_WIND_PAUSE:						// 0x28
			fprintf(strm, "control_byte is HB_WIND_PAUSE, doing nothing\n");
			break;

		case HB_RFTEST:							// 0x1c
			fprintf(strm, "control_byte is HB_RFTEST, doing nothing\n");
			break;

		case HB_ERGO_RUN:							// 0x14
			fprintf(strm, "control_byte is HB_ERGO_RUN, setting to HB_ERGO_PAUSE\n");
			control_byte = HB_ERGO_PAUSE;
			break;

		case HB_ERGO_PAUSE:						// 0x10
			fprintf(strm, "control_byte is HB_ERGO_PAUSE, doing nothing\n");
			break;

		default:
			fprintf(strm, "control_byte is %02x, doing nothing\n", control_byte);
			break;
	}
	fclose(strm);
#else
...
void Computrainer::resume(void)  {
	...
	switch(control_byte)  {
		case HB_WIND_RUN:							// 0x2c
			fprintf(strm, "control_byte is HB_WIND_RUN, doing nothing\n");
			break;

		case HB_WIND_PAUSE:						// 0x28
			fprintf(strm, "control_byte is HB_WIND_PAUSE, setting to HB_WIND_RUN\n");
			//control_byte = HB_WIND_RUN;
			break;
		...
		case HB_ERGO_RUN:							// 0x14
			fprintf(strm, "control_byte is HB_ERGO_RUN, doing nothing\n");
			break;

		case HB_ERGO_PAUSE:						// 0x10
			fprintf(strm, "control_byte is HB_ERGO_PAUSE, setting to HB_ERGO_RUN\n");
	...
	decoder->set_paused(false);												// just clears a bool flag in decoder
...
void Computrainer::set_grade(float _grade)  {
	...
	switch (control_byte)  {
		...
		default:
			control_byte = HB_WIND_RUN;			// for when set_grade is first called
			break;
	}
